...
_bind_mt.__call = function(tb, ...)
    ...
    if type(tb._obj) == 'userdata' and tb._obj == clr.null then 
        dump(tb, "error! invalid eventSystem don't remove ")
        return 
    end
    if tb._obj ~= nil then
        tb._func(tb._obj, ...)
...
--bind function
local function BindFunction(obj, func, eventType)
    local set = {_obj = obj, _func = func, _eventType = eventType}
...
function EventSystem.AddEvent(eventType, obj, func)
    ...
    if not isRepeat then
        table.insert(EventSystem.mEventCaches[eventType], BindFunction(obj, func, eventType))
    end
end
