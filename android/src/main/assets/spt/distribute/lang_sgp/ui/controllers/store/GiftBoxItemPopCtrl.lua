		...
		local confirmCallback = function()
			clr.coroutine(function ()
				...
				if api.success(response) then
					...
					require("ui.common.CustomEvent").ConsumeBlackDiamond("1", price)
...
function GiftBoxItemPopCtrl:BuyGiftBoxSuccess(contents, vip)
    ...
    if self.model:IsSpecial() ~= "" then
        local specialSuit = self.model:IsSpecial()
        local content = lang.trans("buy_bayern_success")
        if specialSuit == "RealMadrid" then
            content = lang.trans("buy_realmadrid_success")
        elseif specialSuit == "Bayern" then
            content = lang.trans("buy_bayern_success")
        end
