...
local RecordCard = require("data.RecordCard")
...
-- 重置某一个球员卡牌的model数据
function PlayerCardsMapModel:ResetCardData(pcid, data)
    assert(type(data) == "table")
...
-- 这里给sgp版打点获得专门的球员时做报送
local function cardInRecordCard(needRecordCardCid)
    local recordCard = RecordCard or {}
    for _cid, v in pairs(recordCard) do
        if _cid == needRecordCardCid then
            return true
        end
    end
end
...
function PlayerCardsMapModel:AddCardData(pcid, data)
    ...
    if cardInRecordCard(cid) then
        luaevt.trig("SDK_Report", "get_assign_card", cid, data.quality)
    end
