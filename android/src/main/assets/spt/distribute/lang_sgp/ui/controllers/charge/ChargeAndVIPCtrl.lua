        ...
        if api.success(resp) then
            ...
            elseif tag == "charge" then
                ...
                if api.success(response) then
                    ...
                    local items = {}
                    local productMap = luaevt.trig("GetProducts") or {}
                    for i, v in ipairs(list) do
                        local item = Object.Instantiate(prefab)
                        local spt = res.GetLuaScript(item)
                        v.BIPrice = v.fee
                        if productMap[v.ID] then
                            v.productName = productMap[v.ID].itemName or v.productName
                            v.priceTag = productMap[v.ID].priceTag
                            v.fee = productMap[v.ID].price or v.fee
                            v.priceLocal = productMap[v.ID].priceLocal
                        end
