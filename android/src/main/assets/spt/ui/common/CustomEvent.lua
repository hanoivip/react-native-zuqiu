...
-- 1.钻石 获得 钻石获得時
function CustomEvent.GetDiamond(get_type, value)
    dump(format("GetDiamond, type:%s, value:%s", tostring(get_type), tostring(value)))
    if luaevt.trig("HasTapjoySDK") then
        require("ui.common.TapjoyCustomEvent").GetDiamond(get_type, value)
    end
    if luaevt.trig("HasMetapsSDK") then
        require("ui.common.MetapsCustomEvent").GetDiamond(get_type, value)
    end
    if luaevt.trig("HasReyunSDK") then
        require("ui.common.ReyunCustomEvent").GetDiamond(get_type, value)
    end
end

-- 2.钻石 消费 钻石消费時
function CustomEvent.ConsumeDiamond(spend_type, value)
    dump(format("ConsumeDiamond, type:%s, value:%s", tostring(spend_type), tostring(value)))
    if luaevt.trig("HasTapjoySDK") then
        require("ui.common.TapjoyCustomEvent").ConsumeDiamond(spend_type, value)
    end
    if luaevt.trig("HasMetapsSDK") then
        require("ui.common.MetapsCustomEvent").ConsumeDiamond(spend_type, value)
    end
    if luaevt.trig("HasReyunSDK") then
        require("ui.common.ReyunCustomEvent").ConsumeDiamond(spend_type, value)
    end    
end
