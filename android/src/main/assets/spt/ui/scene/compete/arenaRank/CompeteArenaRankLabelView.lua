...
local CompeteArenaRankLabelView = class(unity.base, "CompeteArenaRankLabelView")
local DefaultTag = "7"
function CompeteArenaRankLabelView:ctor()
...
function CompeteArenaRankLabelView:start()
end

function CompeteArenaRankLabelView:InitView(rankLabel, labelClickCallBack)
    if type(rankLabel) == "table" then
        self.scrollView:regOnCreateItem(function(scrollSelf, index)
            local prefab = "Assets/CapstonesRes/Game/UI/Scene/Compete/ArenaRank/RankLabelItem.prefab"
...
function CompeteArenaRankLabelView:ChangeSelectTag(tag)
...
function CompeteArenaRankLabelView:GetSelectedTag()
    for tag, spt in pairs(self.btnGroup.menu) do
        if spt.isSelect then return tag end
    end
end

function CompeteArenaRankLabelView:ClearTabs()
...
return CompeteArenaRankLabelView