...
local GuildWarMyDataView = class(unity.base)

function GuildWarMyDataView:ctor()
...
function GuildWarMyDataView:start()
...
function GuildWarMyDataView:OnMenuTypeClick(index)
    if type(self.onMenuItemClick) == "function" then
        self.onMenuItemClick(index)
    end
end

function GuildWarMyDataView:SwitchMenu(index)
    self.menuButtonGroup:selectMenuItem("date" .. index)
end

function GuildWarMyDataView:Close()
...
function GuildWarMyDataView:InitView(model)
    ...
    self.titleText.text = lang.transstr("guildwar_period3", data.list[index].period, lang.transstr("number_" .. level))
