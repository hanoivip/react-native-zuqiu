...
local MistChooseMapItemView = class(unity.base)

function MistChooseMapItemView:ctor()
--------Start_Auto_Generate--------
    ...
    self.applyBtn = self.___ex.applyBtn
    self.applyTxt = self.___ex.applyTxt
    self.usedGo = self.___ex.usedGo
    self.applyTxt = self.___ex.applyTxt
...
function MistChooseMapItemView:start()
    self:RegOnBtn()
end

function MistChooseMapItemView:RegOnBtn()
    self.applyBtn:regOnButtonClick(function ()
        self:ApplyBtnClick()
    end)
end

function MistChooseMapItemView:ApplyBtnClick()
    if self.onApplyClick then
        self.onApplyClick(self.mapData.staticData.id)
    end
end

function MistChooseMapItemView:InitView(mapData, onApplyClick)
    self.mapData = mapData
    self.onApplyClick = onApplyClick
    local staticData = mapData.staticData
    local cacheInfo = mapData.cacheInfo
    local shopMapCountInfo = mapData.shopMapCountInfo
    ...
    local count = tonumber(shopMapCountInfo[tostring(id)])
    self.remainTimeTxt.text = lang.trans("guild_mist_map_count", count)
    -- 是本轮否购买过这张地图
    local isBought = false
    if staticData.mapType == 1 then
        self.remainTimeTxt.text = lang.trans("free_diamond")
    else
        for i, v in ipairs(cacheInfo.mapIds) do
            if v == id then
                self.remainTimeTxt.text = lang.trans("guild_mist_can_use")
                isBought = true
                break
            end
        end
    end
    local applyState = id ~= cacheInfo.mistMapId and (count > 0 or staticData.mapType == 1 or isBought)
    local disableState = id ~= cacheInfo.mistMapId and (count <= 0 and staticData.mapType ~= 1 and (not isBought))
    GameObjectHelper.FastSetActive(self.applyBtn.gameObject, applyState)
    GameObjectHelper.FastSetActive(self.usedGo, id == cacheInfo.mistMapId)
    GameObjectHelper.FastSetActive(self.disableGo, disableState)
end

function MistChooseMapItemView:InitMapPos(staticData)
    self.mapPos = {}
    local mapPosList = staticData.mapPosList
...
function MistChooseMapItemView:CalculateLine(startIndex, endIndex)
...
-- 记录一下连接的两个点
function MistChooseMapItemView:ConnectIndex(startIndex, endIndex)
...
-- 这两个点是否已经连接过了（防止重复连接）
function MistChooseMapItemView:IsConnected(startIndex, endIndex)
