...
function LoginView:SetCurrentServer(server)
    if type(server) == "table" then
        GameObjectHelper.FastSetActive(self.serverItem.gameObject, true)
        local roleName, roleId, roleLvl = "", "", 1
        local hasRole = false
        ...
        self.serverItem:Init(server.displayId, server.name, server.state, hasRole)
        luaevt.trig("SDK_Report", "enter_server", server.id, server.name, roleId, roleName, roleLvl)
    else
        GameObjectHelper.FastSetActive(self.serverItem.gameObject, false)
    end
end
