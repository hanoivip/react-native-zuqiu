...
local CoachSkillBookBoxView = class(unity.base)

function CoachSkillBookBoxView:ctor()
    -- 自己的rectTransform
...
function CoachSkillBookBoxView:InitView(coachItemModel, isShowName, isShowNum, isShowAddNum, isShowDetail)
...
function CoachSkillBookBoxView:start()
...
function CoachSkillBookBoxView:BuildPage()
    self.decorate.overrideSprite = AssetFinder.GetCoachFeatureDecorateIcon(self.itemModel:GetDecoratePicIcon())
    self.imgIcon.overrideSprite = AssetFinder.GetCoachItemIcon(self.itemModel:GetIconIndex(), self.itemModel:GetCoachItemType())
    self.imgQualityBorder.overrideSprite = AssetFinder.GetItemQualityBoard(self.itemModel:GetQuality())
...
function CoachSkillBookBoxView:ResetAddNumSize()
...
--- 设置名称颜色
function CoachSkillBookBoxView:SetNameColor(nameColor, nameShadowColor)
    self.txtName.color = nameColor
    self.nameShadow.effectColor = nameShadowColor
end

--- 设置名称字号
function CoachSkillBookBoxView:SetNumFont(numFont)
    self.isFixNumFont = true
    self.txtAddNum.fontSize = numFont
end

function CoachSkillBookBoxView:OnItemClick()
    res.PushDialog("ui.controllers.itemList.CoachItemDetailCtrl", self.itemModel)
end

return CoachSkillBookBoxView