...
function OBTSerialConsumeItemView:ctor()
    ...
    self.consumeTipText = self.___ex.consumeTipText
...
function OBTSerialConsumeItemView:InitView(obtSerialConsumeModel, index, parentScrollRect)
    ...
    self.consumeTipText.text = self.obtSerialConsumeModel:GetPayDescByIndex(index)
    self.diamondNumberText.text = tostring(self.obtSerialConsumeModel:GetConsumeDiamondNumberByIndex(self.index)) .. "/"
        .. tostring(self.obtSerialConsumeModel:GetRewardConditionByIndex(self.index))
    local rewardParams = {
