...
local OBTCumulativeConsumeItemView = class(unity.base)

function OBTCumulativeConsumeItemView:ctor()
...
function OBTCumulativeConsumeItemView:start()
...
-- state = 0 时表示可以领取奖励
function OBTCumulativeConsumeItemView:InitRewardButtonState(state)
    if state == -1 then
...
function OBTCumulativeConsumeItemView:RefreshTextContentAndButtonState()
...
function OBTCumulativeConsumeItemView:OnRewardBtnClick()
    if self.onRewardBtnClick then
        self:onRewardBtnClick()
    end
end

function OBTCumulativeConsumeItemView:InitView(cumulativeConsumeModel, index, parentScrollRect)
...
return OBTCumulativeConsumeItemView