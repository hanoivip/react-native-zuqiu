...
function CardTrainingItemWithCardView:InitView(cardTrainingMainModel)
    ...
    self.cardView:InitViewWithCount(cid, 0, true, true, self.cardTrainingMainModel)
...
function CardTrainingItemWithCardView:RefreshCoolTime(lvl, subId, time)
...
function CardTrainingItemWithCardView:CheckIsFinish()
    ...
    self:coroutine(function ()
        ...
        if api.success(response) then
            ...
            self.confirmBtn.gameObject:GetComponent(clr.UnityEngine.UI.Button).interactable = data.complete
        end
    end)
end

function CardTrainingItemWithCardView:OnEnterScene()

end

function CardTrainingItemWithCardView:OnExitScene()
    if self.timer then
        self.timer:Destroy()
        self.timer = nil
    end
...
function CardTrainingItemWithCardView:onDestroy()
...
return CardTrainingItemWithCardView